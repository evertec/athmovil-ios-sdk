...
extension KeyedDecodingContainerProtocol {
    ...
    func decodeValueDefault(forKey key: Self.Key) -> NSNumber {
        let value = try? decodeIfPresent(Double.self, forKey: key) ?? 0.0        
        return NSNumber(value: value ?? 0.0)
    }
    ...
    func decodeValueDefault(forKey key: Self.Key) -> ATHMStatus {
        let value = try? decodeIfPresent(ATHMStatus.self.RawValue, forKey: key)?.lowercased() ?? "cancelled"
        return ATHMStatus(rawValue: value ?? "cancelled") ?? .cancelled
    }
