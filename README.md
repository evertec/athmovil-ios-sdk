...
func payWithATHMovil() {
    ...
    /// The code inside onCompleted, onExpired, onCancelled, onPending, onFailed or onException is on the main thread.
    ...
    }, onPending: { [weak self] (payment: ATHMPaymentResponse) in
        /// Handle the response when the payment is pending here
...
* The following optional properties can be used to add additional information to the payment:
  ...
  | `handler` | ATHMPaymentHandler | Yes | Object that handles the response of the payment. The code inside onCompleted, onExpired, onCancelled, onPending, onFailed or onException is on main thread 👁❗️|
...
* It is not necessary to keep a reference of the handler, the SDK will automatically do it. After the SDK processes the URL it will call the correct request through `ATHMHandler` and use the callback `onCompleted`, `onExpired`, `onCancelled`, `onPending`, `onFailed` or `onException` depending on the outcome of the payment.
...
`ATHMPaymentHandler` will now receive an object with the type `ATHMPaymentResponse`. Depending on the outcome of the transaction the closures `onCompleted`, `onExpired`,`onPending`, `onFailed` or `onCancelled` will be automatically called. You will receive an object with the payment information as follows:
...
}, onPending: { [weak self] (response: NSDictionary) in
